function buildValues(
  diff: any[],
  lastComponent: { previousComponent: any } | undefined,
  newString: string | any[],
  oldString: string | any[],
  useLongestToken: any
) {
  ...
  for (; componentPos < componentLen; componentPos++) {
    ...
    if (!component.removed) {
      if (!component.added && useLongestToken) {
        let value = newString.slice(newPos, newPos + component.count) as any;
        value = value.map(function (value: string | any[], i: number) {
        ...
        component.value = diff.join(
          newString.slice(newPos, newPos + component.count) as any
        );
      }
      ...
      component.value = diff.join(
        oldString.slice(oldPos, oldPos + component.count) as any
      );
      oldPos += component.count;
    }
  }
...
Diff.prototype = {
  diff(
    oldString: string | any[],
    newString: string | any[],
    options: { callback?: any; maxEditLength?: any; timeout?: any } = {}
  ) {
  ...
  addToPath(
    path: { lastComponent: any; oldPos: any },
    added: any,
    removed: any,
    oldPosInc: any,
    options: { oneChangePerToken: any }
  ) {
  ...
  equals(
    left: string,
    right: string,
    options: { comparator: (arg0: any, arg1: any) => any; ignoreCase: any }
  ) {
  ...
  tokenize(value: Iterable<unknown> | ArrayLike<unknown>) {
    return Array.from(value);
  },
  join(chars: any[]) {
    return chars.join("");
  },
  postProcess(changeObjects: any) {
